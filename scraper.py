...
if __name__ == "__main__":
    ...
    current = 0
    total = len(records)
    def scrape_records(records, trackers_list):
        print(f"Scraping from {records[0].id} to {records[-1].id}")
        engine, session = get_database()
        ...
        for record in records:
            ...
            record.seeds = 0
            record.leechers = 0
        for tracker in sample(trackers_list, 7):
            try:
                result = scrape(tracker, hashes)
                for record in records:
                    record.seeds += result[record.hash]["seeds"]
    ...
    Parallel(n_jobs=n_jobs)(delayed(scrape_records)(record, my_trackers) for record in divide_chunks(records, 5))
    session.commit()
